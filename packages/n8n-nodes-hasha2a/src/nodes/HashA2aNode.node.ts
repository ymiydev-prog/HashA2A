...
const OPERATIONS: { value: Operation; description: string; endpoint: string; method: 'GET' | 'POST' }[] = [
  {
    value: 'getPrice',
...
export class HashA2aNode implements INodeType {
      ...
      try {
        ...
        switch (operation) {
          case 'getPrice': {
            ...
            body = { asset };
            if (additional.includeAI) body.analyze = true;
            break;
          }
          case 'listAssets': {
            body = {};
          ...
          case 'aggregateMarketData': {
            const query = this.getNodeParameter('query', i) as string;
            const provider = this.getNodeParameter('provider', i) as string;
            const maxProviders = this.getNodeParameter('maxProviders', i, 4) as number;
        ...
        returnData.push({
          json: additional.raw ? (response as IDataObject) : {
            operation,
            data: response,
            timestamp: Date.now(),
          },
